...
&НаСервере
 Процедура СформироватьДиаграммуРаспределенияАттракционов()
 	...
 	РаспределенияАттракционов.Очистить();
 	...
 	Запрос.Текст =
 		...
 		|ИЗ
 		|	РегистрНакопления.ПосещенияАттракционов.Обороты(&НачалоПериода, &КонецПериода,,) 
 		|		КАК ПосещенияАттракционовОбороты
 		|ИТОГИ
 	...
 	Пока ВыборкаВидыАттракционов.Следующий() Цикл
 		...
 		Точка = РаспределенияАттракционов.УстановитьТочку("Виды аттракционов");		
 		Серия = РаспределенияАттракционов.УстановитьСерию(ВыборкаВидыАттракционов.ВидАттракциона);
 		
 		РаспределенияАттракционов.УстановитьЗначение(Точка, Серия, ВыборкаВидыАттракционов.КоличествоПосещений);
 		
 		ВыборкаАттракционы = ВыборкаВидыАттракционов.Выбрать();
 		
 		Пока ВыборкаАттракционы.Следующий() Цикл
 			
 			Точка = РаспределенияАттракционов.УстановитьТочку("Аттракционы");		
 			Серия = РаспределенияАттракционов.УстановитьСерию(ВыборкаАттракционы.Аттракцион);
 			
 			РаспределенияАттракционов.УстановитьЗначение(Точка, Серия, ВыборкаАттракционы.КоличествоПосещений);			
